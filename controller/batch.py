...
def batch_edit(one_piece, start_frame, end_frame, percent, frame_len, xpos, ypos, xscale, yscale, alpha, rot):
    ...
    if percent == 0:
        ...
        if frame_len > 0:
            ...
            if res == tk.NO:
                return
        batch_shift(piece_id, start_frame, end_frame, frame_len, xpos, ypos, xscale, yscale, alpha, rot)
    else:
        percent_shift(piece_id, start_frame, end_frame, frame_len, xscale, yscale, alpha, rot)

    actions_controller.batch_action(
        piece_id, start_frame, end_frame, percent, frame_len, xpos, ypos, xscale, yscale, alpha, rot
    )


def batch_shift(piece_id, start_frame, end_frame, frame_len, xpos, ypos, xscale, yscale, alpha, rot):
    xscale /= 100
    yscale /= 100
    ...
    for fi in range(start_frame, end_frame+1):
        ...
        if fid in pieces_model.pieces.keys():
            for i in range(0, len(pieces_model.pieces[fid])):
                if pieces_model.pieces[fid][i]['id'] == piece_id or not piece_id:
...
def percent_shift(piece_id, start_frame, end_frame, frame_len, xscale, yscale, alpha, rot):
    frame_len /= 100
    xscale /= 100
    yscale /= 100
    alpha /= 100
